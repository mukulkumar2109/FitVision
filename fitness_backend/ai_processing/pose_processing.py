            ...
            if len(lmList) != 0:
                if exercise == "bicep curls" and body_part in ["right arm", "left arm"]:
                    ...
                    if(shoulder_movement>shoulder_movement_threshold or wrist_movement<wrist_movement_threshold):
                        print(f"{shoulder_movement} {wrist_movement}")
                        if not warning_sent:
                    ...
                    # print(f"prev {prev}")
                    if(first):
                        shoulder_movement=150
                        wrist_movement=0
                        first = False
                    else:
                        wrist_movement = max(abs(wrist_coords[1] - prev[1]), abs(wrist_coords[0] - prev[0])) 
                    # print(shoulder_movement)
                    
                    shoulder_movement_threshold = 100
                    wrist_movement_threshold = 20
                    
                    prev = wrist_coords
                    if(wrist_movement>wrist_movement_threshold or shoulder_movement<shoulder_movement_threshold):
                        print(f"{wrist_movement} {shoulder_movement} ")
                        if not warning_sent:
                            warning = f"You are not preforming {exercise}. Should we move to a different exercise?"
                            results["messages"] = warning
                            warning_sent=True
                    ...
                    if per == 100:
                        ...
                        if dir == 0:
                            ...
                            dir = 1
                            if(flag == 1):
                                shoulder_movement = max(abs(shoulder_coords[-1] - prev2[1]), abs(shoulder_coords[-2] - prev2[0])) 
                                prev2[0]=shoulder_coords[-2]
                                prev2[1]=shoulder_coords[-1]
                                flag = 0
                    if per == 0:
                        if dir == 1:
                            count += 0.5
                            dir = 0
                            if(flag==0):
                                shoulder_movement = max(abs(shoulder_coords[1] - prev2[1]), abs(shoulder_coords[0] - prev2[0])) 
                                prev2[0]=shoulder_coords[-2]
                                prev2[1]=shoulder_coords[-1]
                                flag = 1
